...
function renderCube(data) {
  ...
  const sortedGroups = Object.keys(tree).sort((a, b) => a.localeCompare(b));
  ...
  for (const gName of sortedGroups) {
    ...
    html += `
      <tr class="level-1 row-group" data-target=".child-g${gId}" onclick="toggleRow(this)">
        <td style="cursor: pointer;">
          <i class="fa-solid fa-chevron-down toggle-icon" style="color:var(--primary); margin-right:5px;"></i>
          <strong>${gName}</strong>
        </td>
        <td>-</td>
        <td>Consolidado</td>
    ...
    let sgId = 0;
    const sortedSubgroups = Object.keys(G.children).sort((a, b) => a.localeCompare(b));
    
    for (const sgName of sortedSubgroups) {
      const SG = G.children[sgName];
      sgId++;
      html += `
        <tr class="level-2 child-g${gId} row-group" data-target=".child-sg${gId}-${sgId}" onclick="toggleRow(this)">
          <td style="cursor: pointer;">
            <i class="fa-solid fa-chevron-down toggle-icon" style="color:var(--text-muted); margin-left: 20px; margin-right:5px;"></i>
            ${sgName}
          </td>
          <td>-</td>
          <td>Consolidado</td>
      ...
      let itId = 0;
      const sortedItems = Object.keys(SG.children).sort((a, b) => a.localeCompare(b));

      for (const itName of sortedItems) {
        const IT = SG.children[itName];
        itId++;
        html += `
          <tr class="level-3 child-g${gId} child-sg${gId}-${sgId} row-group" data-target=".child-it${gId}-${sgId}-${itId}" onclick="toggleRow(this)" style="display:table-row;">
            <td style="cursor: pointer;">
              <i class="fa-solid fa-chevron-down toggle-icon" style="color:var(--text-muted); font-size: 11px; margin-left: 40px; margin-right: 5px;"></i>
              ${itName}
            </td>
            <td>-</td>
            <td>Consolidado</td>
        ...
        for (const dtName of sortedDetails) {
          ...
          html += `
            <tr class="level-4 child-g${gId} child-sg${gId}-${sgId} child-it${gId}-${sgId}-${itId}" style="display:table-row; font-size: 13px;">
...
function renderAnual(data) {
  ...
  const tree = {
    'RESULTADO FINANCEIRO': {
      rows: {
        'RECEITAS FINANCEIRAS': { totalAnoBrl: 0, totalAnoUsd: 0, meses: {} },
        'DESPESAS FINANCEIRAS': { totalAnoBrl: 0, totalAnoUsd: 0, meses: {} }
      },
      totalMeses: {}
    },
    'OPERAÇÕES DE IMOBILIZADO': {
      rows: {
        'VENDA DE IMOBILIZADO': { totalAnoBrl: 0, totalAnoUsd: 0, meses: {} },
        'DEPRECIAÇÕES': { totalAnoBrl: 0, totalAnoUsd: 0, meses: {} }
      },
      totalMeses: {}
    }
  };
  ...
  for (const r of data) {
    ...
    if (!G.rows[subgrp]) {
      G.rows[subgrp] = { totalAnoBrl: 0, totalAnoUsd: 0, meses: {} };
    ...
    bodyHtml += `
      <tr style="background-color: var(--bg-hover);">
        <td style="font-weight: 600; color: var(--primary);">
          <i class="fa-solid fa-folder-open" style="margin-right: 6px;"></i> ${gName}
        </td>
        <td class="text-right" style="font-weight: 600;">${fmtBRL(gAcumBrl)}</td>`;
    for (const m of mesesArr) {
      bodyHtml += `<td class="text-right" style="font-weight: 600;">${fmtBRL(G.totalMeses[m].brl)}</td>`;
    }
    bodyHtml += `</tr>`;
      ...
      bodyHtml += `
        <tr>
          <td style="padding-left: 24px;">
            <i class="fa-solid fa-arrow-turn-up fa-rotate-90" style="color: var(--text-muted); margin-right: 6px;"></i> ${sgName}
          </td>
          <td class="text-right">${fmtBRL(SG.totalAnoBrl)}</td>`;
      for (const m of mesesArr) {
         bodyHtml += `<td class="text-right">${fmtBRL(SG.meses[m].brl)}</td>`;
      }
      bodyHtml += `</tr>`;
    }

    // Se for o grupo de imobilizado, adicionar o totalizador extra
    if (gName === 'OPERAÇÕES DE IMOBILIZADO') {
      bodyHtml += `
        <tr style="background-color: rgba(255, 255, 255, 0.02);">
          <td style="font-weight: 700; padding-left: 12px;">TOTAL DO RESULTADO NÃO OPERACIONAL</td>
          <td class="text-right" style="font-weight: 700;">${fmtBRL(gAcumBrl)}</td>`;
      for (const m of mesesArr) {
        bodyHtml += `<td class="text-right" style="font-weight: 700;">${fmtBRL(G.totalMeses[m].brl)}</td>`;
      }
      bodyHtml += `</tr>`;
    }
  }
  ...
  bodyHtml += `
    <tr class="total-row" style="background-color: #22c55e !important; color: #fff;">
      <td style="color: #fff;"><strong>(=) RESULTADO LÍQUIDO</strong></td>
      <td class="text-right" style="color: #fff;"><strong>${fmtBRL(grandTotalBrl)}</strong></td>`;
  for (const m of mesesArr) {
    bodyHtml += `<td class="text-right" style="color: #fff;"><strong>${fmtBRL(grandTotal[m].brl)}</strong></td>`;
  }
  bodyHtml += `</tr>`;
...
window.toggleRow = function(rowEl) {
  ...
  children.forEach(child => {
    if (child.style.display === 'none' || child.style.display === '') {
      ...
      if (child.classList.contains('row-group')) {
         const cIcon = child.querySelector('.toggle-icon');
         if (cIcon) {
            cIcon.classList.remove('fa-chevron-down');
            cIcon.classList.add('fa-chevron-right');
         }
         const cTarget = child.getAttribute('data-target');
         document.querySelectorAll(cTarget).forEach(cc => cc.style.display = 'none');
      }
    }
  });
};
...
window.openModalFechamento = async function() {
  document.getElementById('modal-fechamento-bg').style.display = 'flex';
...
window.closeModalFechamento = function() {
  document.getElementById('modal-fechamento-bg').style.display = 'none';
...
window.openNewFechamentoForm = function() {
  ...
  document.getElementById('edit-fi-grupo').value = 'OPERAÇÕES DE IMOBILIZADO';
  document.getElementById('edit-fi-subgrupo').value = '';
  document.getElementById('edit-fi-item').value = '';
...
window.editFechamento = function(id) {
  ...
  document.getElementById('edit-fi-grupo').value = r.FR_GRUPO || '';
  document.getElementById('edit-fi-subgrupo').value = r.FR_SUBGRUPO || '';
